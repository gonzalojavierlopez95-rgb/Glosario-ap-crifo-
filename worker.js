...
export default {
  async fetch(request, env) {
    ...
    // clasificarEstilo() — corrige clasificaciones viejas desactualizadas
    // (palabras cargadas antes de la Fase 3, o antes de algún ajuste en
    // ESTILO_KEYWORDS).
...
// (sin importar su estado) y recalcula su estilo con la lógica actual de
// clasificarEstilo(), reemplazando por completo lo que tenían en
// word_styles. Corrige casos como palabras cargadas antes de que existiera
// la clasificación automática (Fase 3), o antes de un cambio posterior en
// ESTILO_KEYWORDS, que se habían quedado con una etiqueta vieja.
async function handleAdminReclasificarEstilos(request, env) {
  ...
  try {
    ...
    for (const w of palabras) {
      const estilosNuevos = clasificarEstilo(w.palabra + " " + w.significado);
