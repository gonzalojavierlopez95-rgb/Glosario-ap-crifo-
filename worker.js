...
async function handleAportar(request, env) {
  try {
    ...
    if (!moderacionContenido.aprobado) {
      return json({
...
async function moderarContenidoAporte(env, palabra, significado) {
  try {
    const promptSistema = `Sos un moderador de contenido para un glosario colaborativo en español ` +
      ...
      `IMPORTANTE — NO es tu tarea evaluar relevancia temática: NUNCA rechaces un aporte por considerar ` +
      ...
      `glosario. Toda palabra es bienvenida en este glosario sin importar su origen o temática — tu única ` +
      `tarea es filtrar contenido ofensivo, discriminatorio o dañino según los criterios de arriba, nada más. ` +
      `Respondé ÚNICAMENTE con un JSON, sin texto adicional, con este formato exacto: ` +
      `{"aprobado": true} o {"aprobado": false, "motivo": "razón breve en español, dirigida al usuario, explicando qué transgrede el Código Apócrifo"}.`;
    ...
    if (!claudeRes.ok) {
      // Si la API falla, no dejamos pasar sin revisar: mejor pedir que reintente.
      return { aprobado: false, motivo: "No se pudo verificar el aporte ahora. Probá de nuevo en un momento." };
    }
    ...
    try {
      ...
      return { aprobado: false, motivo: "No se pudo verificar el aporte ahora. Probá de nuevo." };
    }

    if (!resultado.aprobado) {
      return { aprobado: false, motivo: resultado.motivo || "Este aporte no cumple con el Código Apócrifo." };
    }

    return { aprobado: true };

  } catch (err) {
    return { aprobado: false, motivo: "Error al verificar el aporte. Probá de nuevo." };
  }
}
