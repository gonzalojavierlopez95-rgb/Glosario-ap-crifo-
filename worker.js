...
async function clasificarEstiloConIA(env, palabra, significado, estilosPrecargados) {
  try {
    ...
    const promptSistema =
      ...
      `Elegí solo los estilos que encajen de verdad. Una palabra puede pertenecer a más de uno, o a ` +
      `ninguno si genuinamente no encaja en ningún estilo existente — en ese caso devolvé una lista ` +
      `vacía, NO fuerces una palabra a entrar en un estilo solo porque no hay otro mejor. En particular, ` +
      `no asignes estilos de temática mística/apócrifa/gótica a una palabra que sea simple jerga ` +
      `cotidiana, moderna o de otro registro, salvo que su significado realmente hable de eso. ` +
      `Respondé ÚNICAMENTE con un JSON, sin texto adicional, con este formato exacto: ` +
