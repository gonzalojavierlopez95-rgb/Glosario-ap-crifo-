...
async function handleAuthSolicitarLink(request, env) {
  try {
    ...
    await env.DB.prepare(
      `INSERT INTO tokens_acceso (token, email, fecha_creacion, fecha_expiracion, usado)
       VALUES (?, ?, ?, ?, 0)`
    ).bind(token, email, ahora.toISOString(), expiracion.toISOString()).run();

    // Nota Fase 4: cuando exista la pantalla de login, este link va a apuntar
    // a esa pantalla (ej. /login.html?token=...) en vez de al endpoint crudo.
    const url = new URL(request.url);
    const linkMagico = `${url.origin}/verificar-login.html?token=${token}`;
    ...
    if (!usuarioReal) {
      ...
      await env.DB.prepare(
        `INSERT INTO usuarios_reales (email, usuario_id, fecha_registro, ultimo_login)
         VALUES (?, ?, ?, ?)`
      ).bind(fila.email, nuevoUsuarioId, ahora, ahora).run();
      usuarioReal = { email: fila.email, usuario_id: nuevoUsuarioId };
    } else {
      await env.DB.prepare(
