...
async function generarExplicacionBaneo(env, motivoAdmin, lang) {
  ...
  const promptSistema =
    ...
    `Contenido que tiene que incluir sí o sí: (1) que fue apartado de la Comunidad Apócrifa por incumplir ` +
    `las normas de convivencia, (2) el motivo concreto que te paso abajo — si hay uno, integralo en tu ` +
    `propia redacción sin copiarlo literal palabra por palabra; si no hay motivo específico, explicá en ` +
    `términos generales que fue por una infracción a las normas de la comunidad, (3) que el Glosario en sí ` +
    `sigue disponible para consultar libremente, lo único restringido es la Comunidad Apócrifa. ` +
