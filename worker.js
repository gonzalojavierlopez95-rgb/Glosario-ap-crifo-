...
// ORIGEN DEL PANEL DE ADMIN — el panel de administración ahora vive en su
// propio Worker/dominio (admin-apocrifo...), separado del glosario, para
// poder instalarse como app independiente en el celular sin que Android
// confunda las dos PWA (mismo dominio = misma app instalada).
// Como quedan en dominios distintos, el navegador exige CORS para que
// admin.html pueda llamar a las APIs /api/comunidad/admin/* de acá.
const ORIGEN_ADMIN_PERMITIDO = "https://admin-apocrifo.gonzalojavierlopez95.workers.dev";

function corsHeaders(request) {
  const origin = request.headers.get("Origin");
  if (origin !== ORIGEN_ADMIN_PERMITIDO) return null;
  return {
    "Access-Control-Allow-Origin": ORIGEN_ADMIN_PERMITIDO,
...
// FASE 9 — cuánto vale cada tipo de evento para el Índice Apócrifo.
// Esquema "balanceado": ver y buscar valen poco (son pasivos), votar vale
// más (es una decisión activa), y contribuir vale mucho más (es el aporte
// de mayor valor para la comunidad). Son solo constantes: se pueden ajustar
// acá sin tocar el resto de la lógica.
const PUNTOS_EVENTO = { ver: 1, buscar: 1, votar: 3, contribuir: 10 };
const TIPOS_EVENTO_VALIDOS = Object.keys(PUNTOS_EVENTO);
...
// FASE CONSUMO A — precio de Claude en USD por millón de tokens, para
// calcular el costo estimado en el panel de administración. Son valores
// de lista pública (no vienen de la API), así que si Anthropic cambia
// precios hay que actualizar esta constante a mano.
const PRECIO_CLAUDE_POR_MILLON = {
  "claude-sonnet-4-6": { entrada: 3.0, salida: 15.0 }
};

// FASE CONSUMO C — valores por defecto de límites/alertas, usados solo
// si todavía no hay fila para esa clave en la tabla limites_config (la
// primera vez, antes de que el admin configure nada desde el panel).
const LIMITES_CONSUMO_DEFAULT = {
  resend_limite_mensual: "100",           // emails de login por mes
  claude_presupuesto_mensual_usd: "0",    // 0 = sin presupuesto definido (semáforo desactivado)
  alerta_porcentaje: "80"                 // aviso configurable, aparte del semáforo fijo
};

// Lee la config de límites desde D1, completando con los valores por
// defecto lo que todavía no esté guardado.
async function leerLimitesConsumo(env) {
  const filas = await env.DB.prepare(`SELECT clave, valor FROM limites_config`).all();
  const config = { ...LIMITES_CONSUMO_DEFAULT };
...
// Semáforo fijo (guía): verde 0–69%, amarillo 70–89%, rojo 90–100%+.
// "alerta_porcentaje" es un aviso aparte, configurable, para advertir
// antes de llegar al límite (ej. a los 80% aunque todavía esté en amarillo).
...
const T = {
  ...
  // ===== ANATEMA APÓCRIFA — apelación de baneo =====
  apelacionMuyCorta: {
    es: "Contá un poco más — necesitamos al menos unas palabras para poder evaluar la apelación.",
    en: "Tell us a bit more — we need at least a few words to evaluate the appeal.",
    pt: "Conte um pouco mais — precisamos de pelo menos algumas palavras para avaliar o recurso."
  },
  apelacionNoBaneado: {
    es: "No figurás como baneado, así que no hay nada que apelar.",
    en: "You don't appear as banned, so there's nothing to appeal.",
    pt: "Você não aparece como banido, então não há nada a recorrer."
  },
  apelacionSinIntentos: {
    es: "Ya usaste todos los intentos de apelación disponibles para este baneo.",
    en: "You've already used all the appeal attempts available for this ban.",
    pt: "Você já usou todas as tentativas de recurso disponíveis para este banimento."
  }
};
...
export default {
  async fetch(request, env) {
    ...
    // Preflight CORS: el navegador manda esto antes de un GET/POST/PATCH/DELETE
    // "cross-origin" con headers custom (como X-Admin-Key). Solo lo contestamos
    // nosotros mismos para rutas /api/*; el resto del sitio sigue igual.
    if (request.method === "OPTIONS" && url.pathname.startsWith("/api/")) {
      return new Response(null, { status: 204, headers: cors || {} });
    }

    const respuesta = await manejarRuta(request, env, url);

    // Si el pedido vino del panel de admin (dominio nuevo) y es una ruta de
    // API, le agregamos los headers CORS a la respuesta ya armada.
    if (cors && url.pathname.startsWith("/api/")) {
  ...
  // FASE 9/10 — Cron Trigger (ver wrangler.jsonc: corre el día 1 de cada mes
  // a las 00:00 UTC). Calcula el ranking del mes que se acaba de cerrar y,
  // a partir de ese ranking, corona automáticamente a la Palabra del Mes.
  async scheduled(event, env, ctx) {
    const ahora = new Date();
    const mesCerrado = new Date(Date.UTC(ahora.getUTCFullYear(), ahora.getUTCMonth() - 1, 1));
    const mesStr = `${mesCerrado.getUTCFullYear()}-${String(mesCerrado.getUTCMonth() + 1).padStart(2, "0")}`;
    ctx.waitUntil((async () => {
      await calcularYGuardarRanking(env, mesStr);
      await seleccionarPalabraDelMes(env, mesStr);
    })());
  }
};

// Todo el ruteo que antes vivía directo en fetch(request, env) — se movió
// a esta función aparte para poder envolver la respuesta con CORS arriba
// sin tener que repetir esa lógica en cada "return" de cada ruta.
async function manejarRuta(request, env, url) {
    ...
    // ANATEMA APÓCRIFA — el Glosario consulta esto apenas arranca la app,
    // para saber si el usuario_id local está baneado y, si lo está, mostrarle
    // la pantalla completa con la explicación que redactó Claude. Pública
    // (sin checkAuth): solo devuelve el estado del propio usuario_id que
    // manda, nada de otros usuarios.
    if (url.pathname === "/api/comunidad/estado-baneo" && request.method === "GET") {
      return handleEstadoBaneo(request, env);
    }

    // ANATEMA APÓCRIFA — el usuario baneado manda su apelación desde la
    // misma pantalla. Claude clasifica si es una apelación real (con algún
    // argumento, aunque sea breve) o si es solo insultos/spam sin
    // contenido, para no llenarle el panel de alertas a Gonzalo.
    if (url.pathname === "/api/comunidad/apelar-baneo" && request.method === "POST") {
      return handleApelarBaneo(request, env);
    }
    ...
    // ===== FASE CONSUMO A — panel de consumo de Claude =====
    if (url.pathname === "/api/comunidad/admin/consumo/claude" && request.method === "GET") {
      return handleAdminConsumoClaude(request, env);
    }

    // Prueba manual: dispara una llamada real y chica a Claude, sin pasar
    // por el frontend ni por el login. Sirve solo para verificar que el
    // registro de consumo (Fase A) funciona. Protegida con la misma clave
    // de admin, aceptada por ?key= para poder pegarla directo en Chrome.
    if (url.pathname === "/api/comunidad/admin/consumo/prueba-claude" && request.method === "GET") {
      return handleAdminPruebaClaude(request, env);
    }

    // ===== FASE CONSUMO B — panel de Login / Resend =====
    if (url.pathname === "/api/comunidad/admin/consumo/resend" && request.method === "GET") {
      return handleAdminConsumoResend(request, env);
    }

    // ===== FASE CONSUMO C — configuración de límites y alertas =====
    if (url.pathname === "/api/comunidad/admin/consumo/config" && request.method === "GET") {
      return handleAdminConsumoConfigGet(request, env);
    }
    if (url.pathname === "/api/comunidad/admin/consumo/config" && request.method === "POST") {
      return handleAdminConsumoConfigPost(request, env);
    }

    // ===== FASE CONSUMO D — historial agrupado por día o mes =====
    if (url.pathname === "/api/comunidad/admin/consumo/historial" && request.method === "GET") {
      return handleAdminConsumoHistorial(request, env);
    }

    // ===== FASE 9 — métricas de uso y ranking mensual =====
    if (url.pathname === "/api/eventos/registrar" && request.method === "POST") {
      return handleRegistrarEvento(request, env);
    }

    if (url.pathname === "/api/ranking/mensual" && request.method === "GET") {
      return handleRankingMensual(request, env);
    }

    // Disparo manual del cálculo, protegido con la clave de admin — sirve
    // para probar sin esperar a que llegue el día 1 del mes que viene.
    if (url.pathname === "/api/comunidad/admin/calcular-ranking" && request.method === "POST") {
      return handleAdminCalcularRanking(request, env);
    }

    // ===== FASE 10 — Palabra del Mes (selección automática) =====
    if (url.pathname === "/api/comunidad/palabra-del-mes" && request.method === "GET") {
      return handlePalabraDelMes(request, env);
    }

    if (url.pathname === "/api/comunidad/historial-palabra-del-mes" && request.method === "GET") {
      return handleHistorialPalabraDelMes(request, env);
    }

    // Disparo manual (admin): recalcula el ranking del mes indicado y elige
    // su Palabra del Mes, sin esperar al Cron del día 1.
    if (url.pathname === "/api/comunidad/admin/elegir-palabra-del-mes" && request.method === "POST") {
      return handleAdminElegirPalabraDelMes(request, env);
    }

    // Recalcula el estilo de TODAS las palabras con la lógica actual de
    // clasificarEstiloConIA() — corrige clasificaciones viejas desactualizadas
    // (palabras cargadas antes de la Fase 3, o antes de algún ajuste de estilos).
    if (url.pathname === "/api/comunidad/admin/reclasificar-estilos" && request.method === "POST") {
      return handleAdminReclasificarEstilos(request, env);
    }

    // Asignación manual de estilos a una palabra puntual — corrección de
    // último recurso desde el panel admin, sin depender de la IA.
    if (url.pathname === "/api/comunidad/admin/asignar-estilos" && request.method === "POST") {
      return handleAdminAsignarEstilos(request, env);
    }

    // FASE CACHÉ — el archivo del Service Worker (y el index.html que lo
    // registra) NUNCA deben quedar guardados en caché del lado del
    // navegador. Si el navegador cachea service-worker.js, deja de pedirle
    // al servidor la versión nueva, y el aviso de "hay una actualización"
    // (el puntito rojo en el menú ⋮) nunca llega a aparecer, aunque ya
    // hayamos desplegado cambios. Por eso, para esos dos archivos puntuales,
    // se fuerza al navegador a revalidar siempre contra el servidor.
    if (url.pathname === "/service-worker.js" || url.pathname === "/" || url.pathname === "/index.html") {
      const respuesta = await env.ASSETS.fetch(request);
      const nuevaRespuesta = new Response(respuesta.body, respuesta);
      nuevaRespuesta.headers.set("Cache-Control", "no-cache, no-store, must-revalidate");
...
function checkAuth(request, env) {
  const url = new URL(request.url);
  // Acepta la clave por header (X-Admin-Key, uso normal) o por query string
  // (?key=..., para poder probar endpoints GET pegando la URL directo en
  // el navegador del celular, sin herramientas como Postman).
...
// FASE CONSUMO A — Panel de consumo de Claude
// ===================================================================
// Registra en D1 los tokens reales que devuelve cada llamada a la API
// de Claude (nunca se estima por caracteres). Se llama después de cada
// fetch a api.anthropic.com, pasándole el `usage` que vino en la
// respuesta. Si el insert falla, NUNCA tira la función que llamó a
// Claude: el registro de consumo es secundario, no puede romper la
// moderación de alias, la clasificación de estilo ni la moderación de
// aportes.
function costoEstimadoClaude(modelo, tokensEntrada, tokensSalida) {
  const precios = PRECIO_CLAUDE_POR_MILLON[modelo];
  if (!precios) return null;
  return (tokensEntrada / 1_000_000) * precios.entrada + (tokensSalida / 1_000_000) * precios.salida;
}

async function registrarUsoClaude(env, { endpoint_origen, modelo, usage }) {
  try {
    if (!usage) return; // sin datos reales de uso, no se inventa nada
    ...
    const costo = costoEstimadoClaude(modelo, tokensEntrada, tokensSalida);
    const fecha = new Date().toISOString();

    await env.DB.prepare(
      `INSERT INTO claude_uso (fecha, endpoint_origen, modelo, tokens_entrada, tokens_salida, tokens_totales, costo_estimado)
       VALUES (?, ?, ?, ?, ?, ?, ?)`
    ).bind(fecha, endpoint_origen, modelo, tokensEntrada, tokensSalida, tokensTotales, costo).run();
  } catch (err) {
    // Solo log implícito vía respuesta 500 si algo más lo pide; acá no
    // relanzamos el error para no afectar el flujo principal (moderación,
    // clasificación de estilo, etc.) que ya recibió su respuesta de Claude.
  }
}
...
// FASE CONSUMO B — Panel de Login / Resend
// ===================================================================
// Un registro por cada intento de envío de email (login por magic-link).
// Separado del consumo de Claude a propósito, para no mezclar dos cuotas
// distintas. "es_usuario_nuevo" se calcula acá mismo, antes de insertar
// la fila: si ese email nunca tuvo un envío exitoso previo, es la primera
// vez que pide acceso. Un mismo usuario puede pedir el link varias veces
// (solicitudes), pero solo cuenta como "nuevo" la primera vez.
async function registrarUsoResend(env, { email, tipo, exitoso }) {
  try {
    const previo = await env.DB.prepare(
      `SELECT id FROM resend_uso WHERE email = ? AND exitoso = 1 LIMIT 1`
    ).bind(email).first();
    const esUsuarioNuevo = exitoso && !previo ? 1 : 0;
    const fecha = new Date().toISOString();

    await env.DB.prepare(
      `INSERT INTO resend_uso (fecha, email, tipo, exitoso, es_usuario_nuevo)
       VALUES (?, ?, ?, ?, ?)`
    ).bind(fecha, email, tipo, exitoso ? 1 : 0, esUsuarioNuevo).run();
  } catch (err) {
    // Igual que con Claude: el registro de consumo nunca puede tumbar el
    // flujo real de login, así que acá el error se ignora silenciosamente.
  }
}
...
async function handleAuthSolicitarLink(request, env) {
  try {
    ...
    await enviarEmailLogin(env, email, linkMagico, lang).catch(async (errEnvio) => {
      await registrarUsoResend(env, { email, tipo: "login", exitoso: false });
      throw errEnvio;
    });
    await registrarUsoResend(env, { email, tipo: "login", exitoso: true });
...
//
// FIX (28/8/2026): agregamos el parámetro contarComoInfraccion (default true).
// Antes, CADA vez que el admin resolvía una alerta ya existente (banear,
// eliminar, eliminar_banear) se llamaba a esta misma función, lo que sumaba
// una infracción NUEVA al mismo usuario y, si el conteo tocaba un múltiplo
// de 3, disparaba OTRA alerta de reincidencia — que el admin volvía a
// resolver, retroalimentando el ciclo indefinidamente (por eso el conteo
// subía solo: 15 → 25 → 30). Ahora, cuando la infracción se registra como
// consecuencia de resolver una alerta (no como una infracción nueva y
// genuina del usuario), se pasa contarComoInfraccion: false — se guarda en
// el historial igual, pero NO se cuenta para el umbral ni dispara alerta.
async function registrarInfraccion(env, { usuario_id, tipo, contenido, motivo, admin, contarComoInfraccion = true }) {
  ...
  if (!contarComoInfraccion) return;
...
async function banearUsuarioConIPs(env, usuario_id, motivo, admin) {
  ...
  // ANATEMA APÓCRIFA — Claude redacta la explicación que va a ver el
  // usuario baneado cuando vuelva a entrar. Si Claude falla por lo que
  // sea, el baneo ya quedó hecho igual (arriba); acá solo se pierde el
  // texto lindo y el frontend cae a un mensaje genérico.
  try {
    const explicacion = await generarExplicacionBaneo(env, motivo, "es");
    if (explicacion) {
      await env.DB.prepare(
        `UPDATE banned_users SET explicacion = ? WHERE usuario_id = ?`
      ).bind(explicacion, usuario_id).run();
    }
  } catch (err) {
    // silencioso a propósito: nunca puede tumbar el baneo en sí.
  }
}

// ANATEMA APÓCRIFA — le pide a Claude que redacte, en la voz del Tribunal
// Apócrifo (tono gótico/ritual pero claro y respetuoso, sin insultar ni
// humillar), la explicación del baneo que va a leer el usuario. "motivo"
// es lo que escribió el admin, opcional — si no hay nada, Claude igual
// redacta un texto genérico pero coherente con el universo. Siempre deja
// en claro que puede seguir consultando el Glosario, solo pierde acceso
// a la Comunidad Apócrifa (aportar, votar, comentar, alias).
async function generarExplicacionBaneo(env, motivoAdmin, lang) {
  const idioma = NOMBRE_IDIOMA_PARA_PROMPT[lang] || NOMBRE_IDIOMA_PARA_PROMPT.es;
  const promptSistema =
    `Sos la voz del Tribunal del Glosario Apócrifo, un glosario colaborativo de fantasía oscura. ` +
    `Un moderador humano acaba de suspenderle a un usuario el acceso a la Comunidad Apócrifa (la parte ` +
    `social del glosario: aportar palabras, votar, comentar, reservar alias). Tu trabajo es redactarle ` +
    `a ese usuario, en ${idioma}, la explicación de su sanción, para que la lea en ` +
    `una pantalla dentro de la app. ` +
    `Tono: ritual, solemne, con la estética gótica/de fantasía oscura del glosario — pero SIN insultar, ` +
    `humillar, agredir ni dirigirte al usuario de forma cruel. Es una sanción administrativa explicada con ` +
    `estilo narrativo, no un ataque personal. Máximo 5 o 6 oraciones. ` +
    `IMPORTANTE — lenguaje neutro: no sabemos si quien lee es varón o mujer, así que NUNCA califiques ` +
    `directamente a la persona con un participio o adjetivo que marque género. PROHIBIDO usar estas ` +
    `palabras (o su forma femenina) referidas a la persona, en cualquier parte del texto: "apartado", ` +
    `"suspendido", "sancionado", "expulsado", "vetado", "excluido", "removido", "separado". Si alguna de ` +
    `esas palabras se te ocurre, reformulá la oración entera hasta que no la necesites. ` +
    `Arrancá la explicación con una frase parecida a esta (podés adaptarla, no copiarla literal): ` +
    `"Tu acceso a la Comunidad Apócrifa fue restringido por incumplir las normas de convivencia..." — ` +
    `notá que el sujeto de esa oración es "tu acceso", no la persona, y por eso no hace falta ningún ` +
    `participio con género. Mantené esa misma lógica en el resto del texto: el sujeto gramatical de ` +
    `cualquier sanción o restricción tiene que ser algo neutro ("tu acceso", "tu participación", "este ` +
    `vínculo", "esta cuenta"), nunca la persona misma seguida de un participio. Usá "vos"/"tu"/"tus" con ` +
    `total libertad en el resto de la redacción. Antes de responder, releé tu propio texto oración por ` +
    `oración y confirmá que ninguna tenga un participio o adjetivo con género referido a la persona; si ` +
    `encontrás una, reescribila. ` +
    `Contenido que tiene que incluir sí o sí: (1) que su participación en la Comunidad Apócrifa fue ` +
    `restringida por incumplir las normas de convivencia, (2) el motivo concreto que te paso abajo — si ` +
    `hay uno, integralo en tu propia redacción sin copiarlo literal palabra por palabra; si no hay motivo ` +
    `específico, explicá en términos generales que fue por una infracción a las normas de la comunidad, ` +
    `(3) que el Glosario en sí sigue disponible para consultar libremente, lo único restringido es la ` +
    `Comunidad Apócrifa. ` +
    `Respondé ÚNICAMENTE con el texto de la explicación en ${idioma}, sin comillas, sin JSON, sin ` +
    `encabezados ni firma.`;

  const contenidoUsuario = motivoAdmin && motivoAdmin.trim()
    ? `Motivo que anotó el moderador: "${motivoAdmin.trim()}"`
    : `El moderador no dejó un motivo específico por escrito.`;
  ...
  await registrarUsoClaude(env, { endpoint_origen: "explicacion_baneo_" + (lang || "es"), modelo: "claude-sonnet-4-6", usage: claudeData.usage });
...
// ANATEMA APÓCRIFA — consulta pública de estado de baneo. La llama el
// Glosario apenas arranca, con el usuario_id local guardado en el
// dispositivo. Si falla algo, respondemos "no baneado" para no dejar a
// nadie afuera de la app por un error nuestro.
async function handleEstadoBaneo(request, env) {
  try {
    const url = new URL(request.url);
    const usuario_id = (url.searchParams.get("usuario_id") || "").trim();
    if (!usuario_id) return json({ baneado: false });
    const lang = idiomaDesdeRequest(request, {});

    const fila = await env.DB.prepare(
      `SELECT explicacion, explicaciones_idiomas, motivo, fecha, apelacion_estado, apelacion_intentos FROM banned_users WHERE usuario_id = ?`
    ).bind(usuario_id).first();

    if (!fila) {
      // ANATEMA APÓCRIFA — no está baneado, pero puede que acabe de ser
      // reintegrado y todavía no se le avisó. Se lo mostramos una sola vez.
      const reintegro = await env.DB.prepare(
        `SELECT id, mensaje FROM reintegros_pendientes WHERE usuario_id = ? ORDER BY fecha DESC LIMIT 1`
      ).bind(usuario_id).first();
      if (reintegro) {
        await env.DB.prepare(`DELETE FROM reintegros_pendientes WHERE id = ?`).bind(reintegro.id).run();
        return json({ baneado: false, reintegrado: true, mensaje: reintegro.mensaje || null });
      }
      return json({ baneado: false });
    }

    // ANATEMA APÓCRIFA — la explicación en español se genera una sola vez,
    // al banear (queda en "explicacion"). Para otros idiomas se genera
    // recién la primera vez que alguien la pide, y se guarda en caché en
    // "explicaciones_idiomas" (JSON por idioma) para no volver a pagarle
    // a Claude por lo mismo en cada visita.
    let explicacionFinal = fila.explicacion || null;

    if (lang !== "es") {
      let cache = {};
      try {
        cache = fila.explicaciones_idiomas ? JSON.parse(fila.explicaciones_idiomas) : {};
      } catch (err) {
        cache = {};
      }

      if (cache[lang]) {
        explicacionFinal = cache[lang];
      } else {
        const generada = await generarExplicacionBaneo(env, fila.motivo, lang);
        if (generada) {
          cache[lang] = generada;
          await env.DB.prepare(
            `UPDATE banned_users SET explicaciones_idiomas = ? WHERE usuario_id = ?`
          ).bind(JSON.stringify(cache), usuario_id).run();
          explicacionFinal = generada;
        }
        // si Claude falla acá, se cae de vuelta a la explicación en
        // español (fila.explicacion) en vez de dejar al usuario sin nada.
      }
    }
    ...
    return json({
      ...
      fecha: fila.fecha,
      apelacionEstado: fila.apelacion_estado || null,
      apelacionIntentos: fila.apelacion_intentos || 0
    });
  } catch (err) {
    return json({ baneado: false });
  }
...
// ANATEMA APÓCRIFA — tope de intentos de apelación por baneo, para que
// alguien insistiendo con insultos no pueda generar alertas infinitas ni
// gastar de más en llamadas a Claude.
const APELACION_MAX_INTENTOS = 1;
const APELACION_MIN_CHARS = 8;
const APELACION_MAX_CHARS = 1500;

// ANATEMA APÓCRIFA — recibe la apelación del usuario baneado, la manda a
// clasificar con Claude y, si parece genuina, la deja como alerta de
// prioridad alta para que Gonzalo la revise desde el panel. Si Claude la
// descarta (insultos, spam, sin argumento), no se genera alerta y se le
// avisa al usuario, descontando un intento.
async function handleApelarBaneo(request, env) {
  try {
    const body = await request.json();
    const usuario_id = (body.usuario_id || "").toString().trim();
    let texto = (body.texto || "").toString().trim();
    const lang = idiomaDesdeRequest(request, body);

    if (!usuario_id) return json({ error: t(lang, "faltaIdentificarUsuario") }, 400);
    if (texto.length < APELACION_MIN_CHARS) {
      return json({ error: t(lang, "apelacionMuyCorta") }, 400);
    }
    if (texto.length > APELACION_MAX_CHARS) {
      texto = texto.slice(0, APELACION_MAX_CHARS);
    }

    const baneo = await env.DB.prepare(
      `SELECT apelacion_estado, apelacion_intentos FROM banned_users WHERE usuario_id = ?`
    ).bind(usuario_id).first();

    if (!baneo) return json({ error: t(lang, "apelacionNoBaneado") }, 404);

    // Ya hay una apelación genuina esperando revisión humana: no se manda
    // otra encima, ni se gasta otra llamada a Claude.
    if (baneo.apelacion_estado === "en_revision") {
      return json({ ok: true, aceptada: true, yaEnviada: true });
    }

    const intentosPrevios = baneo.apelacion_intentos || 0;
    if (intentosPrevios >= APELACION_MAX_INTENTOS) {
      return json({ error: t(lang, "apelacionSinIntentos") }, 429);
    }

    const evaluacion = await evaluarApelacion(env, texto);
    const fecha = new Date().toISOString();
    const nuevoEstado = evaluacion.valida ? "en_revision" : "rechazada_automatica";
    const nuevosIntentos = intentosPrevios + 1;

    await env.DB.prepare(
      `UPDATE banned_users SET apelacion_texto = ?, apelacion_fecha = ?, apelacion_estado = ?, apelacion_intentos = ? WHERE usuario_id = ?`
    ).bind(texto, fecha, nuevoEstado, nuevosIntentos, usuario_id).run();

    if (evaluacion.valida) {
      await crearAlerta(env, {
        usuario_id,
        tipo: "apelacion_baneo",
        contenido: texto,
        motivo: evaluacion.resumen || null,
        prioridad: "alta"
      });
    }
    ...
    return json({
      ok: true,
      aceptada: evaluacion.valida,
      intentosRestantes: Math.max(0, APELACION_MAX_INTENTOS - nuevosIntentos)
    });
  } catch (err) {
    return json({ error: "Error al procesar la apelación", detalle: err.message }, 500);
  }
}

// ANATEMA APÓCRIFA — le pide a Claude que distinga una apelación real
// (aunque sea breve o esté escrita con bronca, con tal de que tenga algún
// argumento o pedido concreto) de un texto que es puro insulto, spam o
// sin sentido. Ante cualquier duda técnica (falla de red, respuesta
// ilegible) se manda igual a revisión humana — nunca se pierde una
// apelación por un problema nuestro.
async function evaluarApelacion(env, texto) {
  const promptSistema =
    `Evaluás apelaciones a baneos en la Comunidad Apócrifa, la parte social de un glosario colaborativo de ` +
    `fantasía oscura. Te paso el texto que escribió un usuario baneado, pidiendo que le revisen el caso. ` +
    `Tenés que decidir si es una apelación genuina o no. ` +
    `ES GENUINA si da algún tipo de explicación, contexto, argumento o pedido concreto de revisión — aunque ` +
    `sea corta, informal, o esté escrita con bronca o mala palabra de por medio. Ejemplo de apelación válida ` +
    `aunque sea una sola línea: "fue mi hermano el que usó mi cuenta, yo no escribí eso" o "no sabía que esa ` +
    `palabra ya existía, pensé que era mía, bórrenla nomás pero desbaneenme". ` +
    `NO ES GENUINA cuando es insultos o agresiones dirigidas al moderador/sistema SIN ningún argumento de ` +
    `fondo, texto sin sentido o ininteligible, spam, una sola palabra suelta sin contexto, amenazas, o algo ` +
    `que directamente no tiene relación con pedir la revisión del baneo. ` +
    `Ante la duda entre las dos categorías, priorizá considerarla GENUINA — el costo de mandar una apelación ` +
    `de más a revisión humana es mucho menor que descartar una apelación real de alguien. ` +
    `Respondé ÚNICAMENTE con un JSON, sin texto adicional, con este formato exacto: ` +
    `{"valida": true, "resumen": "resumen breve en español rioplatense del argumento o pedido del usuario, ` +
    `para que lo lea un moderador humano"} o {"valida": false}.`;
  ...
  try {
    ...
    if (!claudeRes.ok) return { valida: true };

    const claudeData = await claudeRes.json();
    await registrarUsoClaude(env, { endpoint_origen: "evaluar_apelacion", modelo: "claude-sonnet-4-6", usage: claudeData.usage });
    ...
    if (typeof resultado.valida === "boolean") return resultado;
    return { valida: true };
  } catch (err) {
    return { valida: true };
  }
...
async function desbanearUsuario(env, usuario_id, motivo, admin) {
  // ANATEMA APÓCRIFA — antes de borrar la fila de banned_users, rescatamos
  // el motivo original, la explicación que le mostramos al usuario y su
  // apelación (si hizo una), para que ese rastro no se pierda del todo:
  // queda anotado dentro de la infracción "desbanear", que sí es
  // permanente, visible en el detalle del usuario en el panel.
  const baneoPrevio = await env.DB.prepare(
    `SELECT motivo, explicacion, apelacion_texto, apelacion_estado FROM banned_users WHERE usuario_id = ?`
  ).bind(usuario_id).first();
  ...
  let resumenHistorial = null;
  if (baneoPrevio) {
    const partes = [`Motivo del baneo: ${baneoPrevio.motivo || "sin motivo registrado"}`];
    if (baneoPrevio.explicacion) {
      partes.push(`Explicación mostrada al usuario: "${baneoPrevio.explicacion}"`);
    }
    if (baneoPrevio.apelacion_texto) {
      partes.push(`Apelación del usuario (${baneoPrevio.apelacion_estado || "sin estado"}): "${baneoPrevio.apelacion_texto}"`);
    }
    resumenHistorial = partes.join("\n");
  }

  await registrarInfraccion(env, {
    usuario_id,
    tipo: "desbanear",
    contenido: resumenHistorial,
    motivo,
    admin,
    contarComoInfraccion: false
  });

  // ANATEMA APÓCRIFA — dejamos un aviso de "una sola lectura" para que la
  // próxima vez que este usuario abra el Glosario, se entere de que fue
  // reintegrado (y lea el mensaje del moderador, si escribió uno). Se
  // borra solo apenas se le muestra, así no vuelve a aparecer.
  await env.DB.prepare(
    `INSERT INTO reintegros_pendientes (usuario_id, mensaje, fecha) VALUES (?, ?, ?)`
  ).bind(usuario_id, motivo || null, new Date().toISOString()).run();
}
...
async function handleAportar(request, env) {
  try {
    ...
    // FASE 3 → actualizado: clasificación automática de estilo por IA.
    // El usuario no elige el estilo a mano; Claude analiza la palabra + el
    // significado y decide a qué estilo(s) existentes pertenece de verdad
    // (o a ninguno, si no encaja bien en nada — ya no se fuerza todo lo
    // no reconocido dentro de un estilo por descarte).
    const estilosDetectados = await clasificarEstiloConIA(env, palabra.trim(), significado.trim());
...
async function handleAliasModerar(request, env) {
  try {
    ...
    const claudeData = await claudeRes.json();
    await registrarUsoClaude(env, { endpoint_origen: "alias", modelo: "claude-sonnet-4-6", usage: claudeData.usage });
    const textoRespuesta = (claudeData.content || [])
      .filter(b => b.type === "text")
      .map(b => b.text)
      .join("")
...
// Clasificación de estilo por IA — método principal desde esta fase.
// Le mostramos a Claude la lista de estilos que existen HOY en la base
// (así se adapta solo si Gonzalo agrega estilos nuevos, sin tocar código)
// y le pedimos que entienda el registro/tema real de la palabra, en vez
// de depender de que aparezca literalmente alguna palabra clave.
// Puede devolver una lista vacía si genuinamente no encaja en nada — ya
// no se fuerza a que toda palabra "no reconocida" caiga en un estilo por
// descarte, que era el problema con el diccionario de keywords viejo.
// Si la IA falla por cualquier motivo (red, JSON inválido, etc.), cae a
// clasificarEstilo() por keywords como red de seguridad, para que un
// aporte nunca se quede sin intentar clasificar por un problema puntual
// de la API.
async function clasificarEstiloConIA(env, palabra, significado, estilosPrecargados) {
  try {
    const estilos =
      estilosPrecargados ||
      (
        await env.DB.prepare(
          `SELECT id, nombre, categoria_padre FROM styles ORDER BY categoria_padre, nombre`
        ).all()
      ).results;

    if (!estilos || estilos.length === 0) {
      return clasificarEstilo(palabra + " " + significado);
    }

    const listaEstilos = estilos
      .map((e) => `${e.id}. ${e.nombre} (categoría: ${e.categoria_padre})`)
      .join("\n");

    const promptSistema =
      `Sos el clasificador temático del Glosario Apócrifo, un diccionario colaborativo de palabras ` +
      `y definiciones inventadas o adaptadas por su comunidad. Tu tarea es, dada una palabra y su ` +
      `significado, elegir a cuál o cuáles de los siguientes estilos pertenece REALMENTE por su tema, ` +
      `registro o tono — no busques coincidencias literales de texto, entendé el sentido real de la ` +
      `palabra. Estilos disponibles (id, nombre, categoría):\n${listaEstilos}\n\n` +
      `Sé conservador con la cantidad de estilos que elegís. Lo normal es UN solo estilo: el que mejor ` +
      `describe de qué trata la palabra en esencia. Sumá un segundo estilo únicamente si la palabra es ` +
      `de verdad, y en partes iguales, sobre dos temas distintos — no alcanza con que la definición ` +
      `roce o mencione de pasada un tema relacionado. Por ejemplo, una palabra que sea centralmente ` +
      `sobre una criatura gótica (vampiros, fantasmas, etc.) va en el estilo gótico, aunque su ` +
      `definición también use palabras como "alma" o "apócrifo" de forma incidental — esas menciones ` +
      `de paso no ameritan sumar un estilo más. Reservá 3 estilos o más solo para casos donde la palabra ` +
      `sea, sin exagerar, igual de central a cada uno de esos temas. Ante la duda entre asignar un ` +
      `estilo de más o de menos, preferí quedarte corto. Y a la inversa: elegí NINGÚN estilo (lista ` +
      `vacía) si genuinamente no encaja bien en nada — no fuerces una palabra a entrar en un estilo ` +
      `solo porque no hay otro mejor. En particular, no asignes estilos de temática mística/apócrifa/ ` +
      `gótica a una palabra que sea simple jerga cotidiana, moderna o de otro registro, salvo que su ` +
      `significado realmente hable de eso. ` +
      `Respondé ÚNICAMENTE con un JSON, sin texto adicional, con este formato exacto: ` +
      `{"estilos": [id1, id2]} (usando los ids numéricos de la lista de arriba; el array puede tener ` +
      `0, 1 o más elementos, pero recordá que lo normal y esperado es que tenga solo 1).`;
    ...
    const claudeData = await claudeRes.json();
    await registrarUsoClaude(env, { endpoint_origen: "clasificar_estilo", modelo: "claude-sonnet-4-6", usage: claudeData.usage });
    const textoRespuesta = (claudeData.content || [])
      .filter((b) => b.type === "text")
      .map((b) => b.text)
      .join("")
    ...
    const resultado = JSON.parse(textoRespuesta);
    const idsValidos = new Set(estilos.map((e) => e.id));
    return (resultado.estilos || []).filter((id) => idsValidos.has(id));
  } catch (err) {
    return clasificarEstilo(palabra + " " + significado);
  }
}

// Clasificación por keywords — se mantiene como red de seguridad para
// cuando la IA no está disponible (ver clasificarEstiloConIA arriba).
function clasificarEstilo(texto) {
  ...
  // Ya no se fuerza nada a "Apócrifas" por default: si ninguna keyword
  // matcheó, mejor dejarla sin estilo que taggearla mal (ver Fase 10.2).
  return detectados;
}

// ADMIN — reclasificación masiva de estilos. Recorre TODAS las palabras
// (sin importar su estado) y recalcula su estilo con clasificarEstiloConIA()
// (con clasificarEstilo() por keywords como red de seguridad si la IA
// falla), reemplazando por completo lo que tenían en word_styles. Corrige
// casos como palabras cargadas antes de que existiera la clasificación
// automática (Fase 3), o clasificadas con una versión anterior del método
// (por ejemplo, el viejo fallback por keywords que forzaba todo lo no
// reconocido dentro de "Apócrifas").
async function handleAdminReclasificarEstilos(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    const { results: palabras } = await env.DB.prepare(
      `SELECT id, palabra, significado FROM words`
    ).all();

    // Se precarga la lista de estilos UNA sola vez acá afuera (en vez de
    // que cada llamada a clasificarEstiloConIA la vuelva a pedir) para no
    // hacer una consulta a D1 de más por cada palabra.
    const { results: estilosDisponibles } = await env.DB.prepare(
      `SELECT id, nombre, categoria_padre FROM styles ORDER BY categoria_padre, nombre`
    ).all();

    let actualizadas = 0;
    for (const w of palabras) {
      const estilosNuevos = await clasificarEstiloConIA(env, w.palabra, w.significado, estilosDisponibles);
      await env.DB.prepare(`DELETE FROM word_styles WHERE word_id = ?`).bind(w.id).run();
      for (const styleId of estilosNuevos) {
        await env.DB.prepare(
          `INSERT INTO word_styles (word_id, style_id) VALUES (?, ?)`
        ).bind(w.id, styleId).run();
      }
      actualizadas += 1;
    }

    return json({ ok: true, palabras_reclasificadas: actualizadas });
  } catch (err) {
    return json({ error: "Error al reclasificar estilos", detalle: err.message }, 500);
  }
}

// ADMIN — asignación manual de estilos a una palabra puntual. Pensado
// como la vía de corrección de último recurso: si la IA se equivoca en
// algún caso raro, el admin puede pisar el resultado a mano desde el
// panel, sin depender de reformular keywords ni de reintentar la IA.
// Reemplaza por completo los estilos de la palabra — mandar un array
// vacío la deja sin ningún estilo asignado.
async function handleAdminAsignarEstilos(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    ...
    const estilos = Array.isArray(body.estilos) ? body.estilos : [];

    if (!wordId) {
      return json({ error: "Falta el id de la palabra" }, 400);
    }

    await env.DB.prepare(`DELETE FROM word_styles WHERE word_id = ?`).bind(wordId).run();
    ...
    return json({ ok: true, id: wordId, estilos });
  } catch (err) {
    return json({ error: "Error al asignar estilos", detalle: err.message }, 500);
  }
}
...
async function moderarContenidoAporte(env, palabra, significado, lang) {
  try {
    ...
    const claudeData = await claudeRes.json();
    await registrarUsoClaude(env, { endpoint_origen: "moderacion_aporte", modelo: "claude-sonnet-4-6", usage: claudeData.usage });
    const textoRespuesta = (claudeData.content || [])
      .filter((b) => b.type === "text")
      .map((b) => b.text)
      .join("")
...
async function handleBanearUsuario(request, env) {
  ...
  try {
    const body = await request.json();
    let { usuario_id, motivo } = body;
    const email = (body.email || "").toString().trim().toLowerCase();

    // FASE 6: banear también por email — más confiable que usuario_id o
    // IP, porque no cambia si la persona borra caché o cambia de celular.
    if (!usuario_id && email) {
      const cuenta = await env.DB.prepare(
        `SELECT usuario_id FROM usuarios_reales WHERE email = ?`
      ).bind(email).first();
      if (!cuenta) {
        return json({ error: "No existe ninguna cuenta logueada con ese email" }, 404);
      }
      usuario_id = cuenta.usuario_id;
    }

    if (!usuario_id) return json({ error: "Falta usuario_id o email" }, 400);

    // Si ya estaba baneado (ej: doble clic, o lo intentan banear de nuevo
    // sin haberlo reintegrado antes), no hacemos nada nuevo: ni sumamos
    // otra infracción, ni le pagamos a Claude por redactar la explicación
    // de vuelta. El baneo que ya existe queda como está.
    const yaBaneado = await env.DB.prepare(
      `SELECT id FROM banned_users WHERE usuario_id = ?`
    ).bind(usuario_id).first();
    if (yaBaneado) {
      return json({ ok: true, usuario_id, yaEstabaBaneado: true });
    }
    ...
    return json({ ok: true, usuario_id });
  } catch (err) {
...
async function handleEliminarYBanear(request, env) {
  ...
  try {
    ...
    // Igual que en el baneo directo: si el usuario ya estaba baneado (tenía
    // otro aporte pendiente y ya lo habían baneado por ese), no lo baneamos
    // de nuevo ni gastamos otra llamada a Claude — el aporte se elimina
    // igual, pero el baneo que ya existe queda como está.
    const yaBaneado = await env.DB.prepare(
      `SELECT id FROM banned_users WHERE usuario_id = ?`
    ).bind(usuarioObjetivo).first();
...
async function handleResolverAlerta(request, env) {
  ...
  try {
    ...
    if (!id || !["eliminar", "banear", "eliminar_banear", "descartar", "desbanear"].includes(accion)) {
      return json({ error: "Datos inválidos" }, 400);
    }
    ...
    // ANATEMA APÓCRIFA — resolver una apelación de baneo con "desbanear":
    // el moderador le da la razón al usuario y lo reintegra a la Comunidad.
    if (accion === "desbanear") {
      await desbanearUsuario(env, usuarioObjetivo, motivo || "Apelación aceptada", "admin");
    }
    ...
    if (accion === "banear" || accion === "eliminar_banear") {
      ...
      // FIX: resolver una alerta ya existente no es una infracción nueva del
      // usuario, es una acción del admin — no debe sumar al contador ni
      // disparar otra alerta de reincidencia (eso causaba un loop infinito).
      await registrarInfraccion(env, {
        usuario_id: usuarioObjetivo,
        tipo: "banear",
        contenido: null,
    ...
    // ANATEMA APÓCRIFA — si se descarta una alerta de apelación (el baneo
    // se mantiene), el estado en banned_users deja de ser "en_revision"
    // para que el usuario pueda ver que fue resuelta (y, si le quedan
    // intentos, volver a apelar más adelante si la situación cambia).
    if (alerta.tipo === "apelacion_baneo" && accion === "descartar") {
      await env.DB.prepare(
        `UPDATE banned_users SET apelacion_estado = 'rechazada' WHERE usuario_id = ?`
      ).bind(usuarioObjetivo).run();
    }
...
async function handleAdminBuscar(request, env) {
  ...
  try {
    ...
    const { results: palabras } = await env.DB.prepare(`
      SELECT w.id, w.palabra, w.significado, w.autor_id, w.usuario_id, w.estado, w.fecha_creacion,
             (SELECT GROUP_CONCAT(ws.style_id) FROM word_styles ws WHERE ws.word_id = w.id) AS estilos_ids
      FROM words w
      WHERE w.palabra LIKE ? OR w.significado LIKE ? OR w.autor_id LIKE ? OR w.usuario_id LIKE ?
      ORDER BY w.fecha_creacion DESC
      LIMIT 50
  ...
  try {
    ...
    let sql = `
      SELECT
        ...
        (SELECT email FROM usuarios_reales ur WHERE ur.usuario_id = a.usuario_id) AS email,
    ...
    const binds = [];
    if (q) {
      // FASE 6: la búsqueda de usuarios ahora también encuentra por email.
      sql += ` WHERE a.usuario_id LIKE ? OR a.alias_texto LIKE ?
               OR EXISTS (SELECT 1 FROM usuarios_reales ur WHERE ur.usuario_id = a.usuario_id AND ur.email LIKE ?) `;
      binds.push(`%${q}%`, `%${q}%`, `%${q}%`);
    }
...
async function handleAdminUsuarioDetalle(request, env) {
  ...
  try {
    ...
    // FASE 6: email real de la cuenta, si ya se logueó alguna vez.
    const cuentaReal = await env.DB.prepare(
      `SELECT email FROM usuarios_reales WHERE usuario_id = ?`
    ).bind(usuarioId).first();
    ...
    return json({
      ...
      email: cuentaReal ? cuentaReal.email : null,
...
async function handleAdminBaneados(request, env) {
  ...
  try {
    const { results } = await env.DB.prepare(`
      SELECT
        b.usuario_id, b.motivo, b.fecha, b.admin, b.explicacion,
        b.apelacion_texto, b.apelacion_estado, b.apelacion_fecha,
        (SELECT alias_texto FROM aliases al WHERE al.usuario_id = b.usuario_id ORDER BY al.fecha DESC LIMIT 1) AS alias,
        (SELECT email FROM usuarios_reales ur WHERE ur.usuario_id = b.usuario_id) AS email
      FROM banned_users b
...
// Totales de consumo real de Claude para mostrar en el admin: hoy, este
// mes, histórico total, y la última llamada registrada. Todo sale de
// claude_uso, que se alimenta con tokens reales devueltos por la API
// (ver registrarUsoClaude arriba) — nunca estimaciones por caracteres.
async function handleAdminConsumoClaude(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    ...
    const totales = async (desde) => {
      const fila = await env.DB.prepare(
        `SELECT COUNT(*) AS solicitudes,
                COALESCE(SUM(tokens_entrada), 0) AS tokens_entrada,
                COALESCE(SUM(tokens_salida), 0) AS tokens_salida,
                COALESCE(SUM(tokens_totales), 0) AS tokens_totales,
                COALESCE(SUM(costo_estimado), 0) AS costo_estimado
         FROM claude_uso
         ${desde ? "WHERE fecha >= ?" : ""}`
      ).bind(...(desde ? [desde] : [])).first();
      return fila;
    };

    const hoy = await totales(inicioHoy);
    const esteMes = await totales(inicioMes);
    const historico = await totales(null);

    const ultimaLlamada = await env.DB.prepare(
      `SELECT fecha, endpoint_origen, modelo, tokens_entrada, tokens_salida, tokens_totales, costo_estimado
       FROM claude_uso ORDER BY fecha DESC LIMIT 1`
    ).first();

    // FASE CONSUMO C — semáforo sobre el gasto del mes, solo si hay un
    // presupuesto mensual configurado (> 0). Si no se configuró, no se
    // inventa un límite: el semáforo queda en null.
    const limites = await leerLimitesConsumo(env);
    const presupuesto = parseFloat(limites.claude_presupuesto_mensual_usd) || 0;
    const alertaPorcentaje = parseFloat(limites.alerta_porcentaje) || 80;
    let porcentajeUsado = null;
    if (presupuesto > 0) {
      porcentajeUsado = (esteMes.costo_estimado / presupuesto) * 100;
    }
    ...
    return json({
      ...
      este_mes: esteMes,
      historico,
      ultima_llamada: ultimaLlamada || null,
      semaforo: {
        presupuesto_mensual_usd: presupuesto > 0 ? presupuesto : null,
        porcentaje_usado: porcentajeUsado,
        color: calcularSemaforo(porcentajeUsado),
        alerta: porcentajeUsado !== null && porcentajeUsado >= alertaPorcentaje
      }
    });
  } catch (err) {
    return json({ error: "Error al obtener el consumo de Claude", detalle: err.message }, 500);
  }
}

// Prueba manual (solo Fase A): manda un mensaje trivial a Claude, registra
// el consumo real con registrarUsoClaude(), y devuelve el resultado —
// para confirmar que el panel de consumo funciona sin depender del
// frontend ni del login por email.
async function handleAdminPruebaClaude(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    ...
    if (!claudeRes.ok) {
      ...
      return json({ error: "Claude respondió con error", detalle }, 502);
    }

    const claudeData = await claudeRes.json();
    await registrarUsoClaude(env, { endpoint_origen: "prueba_admin", modelo, usage: claudeData.usage });
    ...
    return json({ ok: true, respuesta_claude: textoRespuesta, usage: claudeData.usage });
  } catch (err) {
    return json({ error: "Error en la prueba de Claude", detalle: err.message }, 500);
  }
}
...
// Totales de envíos de email para el admin: hoy, este mes, histórico,
// y la distinción entre usuarios nuevos y solicitudes de login (un mismo
// usuario puede pedir el link varias veces sin ser "nuevo" cada vez).
async function handleAdminConsumoResend(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    ...
    const totales = async (desde) => {
      const fila = await env.DB.prepare(
        `SELECT
           ...
           COALESCE(SUM(CASE WHEN exitoso = 1 THEN 1 ELSE 0 END), 0) AS emails_enviados,
           COALESCE(SUM(CASE WHEN exitoso = 0 THEN 1 ELSE 0 END), 0) AS emails_fallidos,
           COALESCE(SUM(CASE WHEN es_usuario_nuevo = 1 THEN 1 ELSE 0 END), 0) AS usuarios_nuevos
         FROM resend_uso
         ${desde ? "WHERE fecha >= ?" : ""}`
      ).bind(...(desde ? [desde] : [])).first();
      return fila;
    ...
    const ultimoEnvio = await env.DB.prepare(
      `SELECT fecha, tipo, exitoso, es_usuario_nuevo FROM resend_uso ORDER BY fecha DESC LIMIT 1`
    ).first();

    // FASE CONSUMO C — semáforo sobre los emails enviados este mes contra
    // el límite mensual configurado (ej. el límite gratuito de Resend).
    const limites = await leerLimitesConsumo(env);
    const limite = parseFloat(limites.resend_limite_mensual) || 0;
    const alertaPorcentaje = parseFloat(limites.alerta_porcentaje) || 80;
    let porcentajeUsado = null;
    if (limite > 0) {
      porcentajeUsado = (esteMes.emails_enviados / limite) * 100;
    }
    ...
    return json({ error: "Error al obtener el consumo de Resend", detalle: err.message }, 500);
  }
}
...
// Devuelve la config actual (con los valores por defecto si todavía no
// se guardó nada). Las claves válidas son las de LIMITES_CONSUMO_DEFAULT.
async function handleAdminConsumoConfigGet(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    const config = await leerLimitesConsumo(env);
    return json({ config });
  } catch (err) {
    return json({ error: "Error al obtener la configuración", detalle: err.message }, 500);
  }
}

// Actualiza uno o más límites desde el admin. Body esperado:
// { "resend_limite_mensual": "150", "alerta_porcentaje": "85" }
// Solo se guardan las claves conocidas (LIMITES_CONSUMO_DEFAULT), para
// evitar que se cargue basura en la tabla por error de tipeo.
async function handleAdminConsumoConfigPost(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    const body = await request.json();
    const clavesValidas = Object.keys(LIMITES_CONSUMO_DEFAULT);
    const actualizadas = [];
    ...
    for (const clave of clavesValidas) {
      ...
      await env.DB.prepare(
        `INSERT INTO limites_config (clave, valor) VALUES (?, ?)
    ...
    const config = await leerLimitesConsumo(env);
    return json({ ok: true, actualizadas, config });
  } catch (err) {
    return json({ error: "Error al guardar la configuración", detalle: err.message }, 500);
  }
}
...
// FASE CONSUMO D — historial agrupado por día o mes
// ===================================================================
// Junta claude_uso y resend_uso agrupados por fecha, listo para graficar.
// "periodo" define el agrupamiento: 'dia' agrupa por los primeros 10
// caracteres de la fecha ISO (YYYY-MM-DD), 'mes' por los primeros 7
// (YYYY-MM). Se limita la cantidad de puntos para no devolver historiales
// gigantes (30 días o 12 meses por defecto).
async function handleAdminConsumoHistorial(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    const url = new URL(request.url);
    const periodo = url.searchParams.get("periodo") === "mes" ? "mes" : "dia";
    const largoFecha = periodo === "mes" ? 7 : 10;
    const limite = periodo === "mes" ? 12 : 30;

    const claudeRows = await env.DB.prepare(
      `SELECT substr(fecha, 1, ${largoFecha}) AS periodo,
    ...
    const resendRows = await env.DB.prepare(
      `SELECT substr(fecha, 1, ${largoFecha}) AS periodo,
              COALESCE(SUM(CASE WHEN exitoso = 1 THEN 1 ELSE 0 END), 0) AS emails_enviados,
              COALESCE(SUM(CASE WHEN es_usuario_nuevo = 1 THEN 1 ELSE 0 END), 0) AS usuarios_nuevos
       FROM resend_uso
    ...
    // Se invierte para que quede en orden cronológico ascendente (más
    // fácil de graficar de izquierda a derecha).
    const claude = (claudeRows.results || []).reverse();
    const resend = (resendRows.results || []).reverse();

    return json({ periodo, claude, resend });
  } catch (err) {
    return json({ error: "Error al obtener el historial de consumo", detalle: err.message }, 500);
  }
}

// ===================================================================
// FASE 9 — Métricas de uso y ranking mensual (Índice Apócrifo)
// ===================================================================

// Registra un evento de uso en bruto. Se llama desde el frontend de forma
// "fire-and-forget" (no bloquea la interacción del usuario si falla).
// - tipo_evento: 'ver' | 'buscar' | 'votar' | 'contribuir'
// - palabra_id: opcional (no aplica a 'buscar', que es general)
async function handleRegistrarEvento(request, env) {
  try {
    const body = await request.json();
    const { usuario_id } = body;
    const fecha = new Date().toISOString();

    // Soporta un evento único o, para 'ver', un lote de varias palabras
    // (así una sola pantalla con 10 tarjetas no dispara 10 requests).
    const eventos = Array.isArray(body.palabra_ids)
      ? body.palabra_ids.map((id) => ({ tipo_evento: body.tipo_evento, palabra_id: id }))
      : [{ tipo_evento: body.tipo_evento, palabra_id: body.palabra_id ?? null }];

    const validos = eventos.filter((e) => TIPOS_EVENTO_VALIDOS.includes(e.tipo_evento));
    if (validos.length === 0) {
      return json({ error: "tipo_evento inválido" }, 400);
    }

    // Insertamos de a uno — el volumen esperado es bajo y así evitamos
    // depender de sintaxis de batch específica de D1.
    for (const ev of validos) {
      await env.DB.prepare(
        `INSERT INTO usage_events (tipo_evento, palabra_id, usuario_id, fecha) VALUES (?, ?, ?, ?)`
      ).bind(ev.tipo_evento, ev.palabra_id ?? null, usuario_id || null, fecha).run();
    }

    return json({ ok: true, registrados: validos.length });
  } catch (err) {
    // Este endpoint nunca debe romper la experiencia del usuario: un fallo
    // acá solo significa que no se contó una métrica, no un error visible.
    return json({ error: "Error al registrar evento", detalle: err.message }, 500);
  }
}

// Devuelve el ranking ya calculado de un mes (por defecto, el actual).
async function handleRankingMensual(request, env) {
  try {
    const url = new URL(request.url);
    const mes = url.searchParams.get("mes") || mesActualUTC();
    const limite = Math.min(parseInt(url.searchParams.get("limite") || "20", 10) || 20, 100);

    const { results } = await env.DB.prepare(`
      SELECT r.palabra_id, r.puntaje, r.posicion, w.palabra, w.significado, w.autor_id
      FROM monthly_rankings r
      JOIN words w ON w.id = r.palabra_id
      WHERE r.mes = ?
      ORDER BY r.posicion ASC
      LIMIT ?
    `).bind(mes, limite).all();

    return json({ mes, ranking: results });
  } catch (err) {
    return json({ error: "Error al obtener el ranking", detalle: err.message }, 500);
  }
}

// Disparo manual (admin) del cálculo — para probar sin esperar al Cron.
async function handleAdminCalcularRanking(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    const body = await request.json().catch(() => ({}));
    const mes = body.mes || mesActualUTC();
    const resultado = await calcularYGuardarRanking(env, mes);
    return json({ ok: true, mes, ...resultado });
  } catch (err) {
    return json({ error: "Error al calcular el ranking", detalle: err.message }, 500);
  }
}
...
// Motor del cálculo: agrupa usage_events del mes indicado, aplica los
// puntos de PUNTOS_EVENTO, ordena y guarda el resultado en monthly_rankings.
// Solo entran palabras que tuvieron al menos un evento ese mes.
// Es idempotente: si se vuelve a correr para el mismo mes, reemplaza el
// resultado anterior en vez de duplicarlo.
async function calcularYGuardarRanking(env, mes) {
  const [anio, mesNum] = mes.split("-").map(Number);
  const inicio = new Date(Date.UTC(anio, mesNum - 1, 1)).toISOString();
  const fin = new Date(Date.UTC(mesNum === 12 ? anio + 1 : anio, mesNum === 12 ? 0 : mesNum, 1)).toISOString();

  const casosPuntaje = TIPOS_EVENTO_VALIDOS
    .map((tipo) => `WHEN '${tipo}' THEN ${PUNTOS_EVENTO[tipo]}`)
    .join(" ");

  const { results } = await env.DB.prepare(`
    SELECT palabra_id, SUM(CASE tipo_evento ${casosPuntaje} ELSE 0 END) AS puntaje
    FROM usage_events
    WHERE fecha >= ? AND fecha < ? AND palabra_id IS NOT NULL
    GROUP BY palabra_id
  ...
  // Reemplaza el ranking previo de ese mes (por si se recalcula a mano).
  await env.DB.prepare(`DELETE FROM monthly_rankings WHERE mes = ?`).bind(mes).run();

  let posicion = 0;
  for (const fila of results) {
    posicion += 1;
    await env.DB.prepare(
      `INSERT INTO monthly_rankings (palabra_id, mes, puntaje, posicion, calculado_en) VALUES (?, ?, ?, ?, ?)`
    ).bind(fila.palabra_id, mes, fila.puntaje, posicion, calculadoEn).run();
  }

  return { palabras_rankeadas: results.length };
}

// ===================================================================
// FASE 10 — Palabra del Mes (selección automática a partir del ranking)
// ===================================================================

// Toma la posición 1 del ranking mensual ya calculado (ver FASE 9) y la
// guarda como la Palabra del Mes de ese período, junto con el autor_id
// que ya tenía la palabra en `words` (mismo texto de alias que se ve en
// el resto de la Comunidad, no hace falta cruzar contra `aliases`).
// Es idempotente: si ya existía una selección para ese mes, la reemplaza
// (por si se recalcula el ranking a mano y cambia el ganador).
async function seleccionarPalabraDelMes(env, mes) {
  const ganador = await env.DB.prepare(`
    SELECT r.palabra_id, r.puntaje, w.autor_id
    FROM monthly_rankings r
    JOIN words w ON w.id = r.palabra_id
    WHERE r.mes = ? AND r.posicion = 1
  `).bind(mes).first();

  if (!ganador) {
    // Mes sin ningún evento de uso registrado: no hay a quién coronar.
    return { seleccionada: false };
  }

  const fecha = new Date().toISOString();
  await env.DB.prepare(`DELETE FROM palabra_del_mes WHERE mes = ?`).bind(mes).run();
  await env.DB.prepare(`
    INSERT INTO palabra_del_mes (mes, palabra_id, autor_id, puntaje, fecha_seleccion)
    VALUES (?, ?, ?, ?, ?)
  `).bind(mes, ganador.palabra_id, ganador.autor_id, ganador.puntaje, fecha).run();

  return {
    seleccionada: true,
    palabra_id: ganador.palabra_id,
    autor_id: ganador.autor_id,
    puntaje: ganador.puntaje
  };
}

// Palabra del Mes vigente para mostrar en la tarjeta "Del mes". Sin ?mes,
// devuelve la más reciente ya coronada (no necesariamente el mes calendario
// actual: ese recién se corona el día 1 del mes siguiente, cuando el Cron
// cierra el período anterior).
async function handlePalabraDelMes(request, env) {
  try {
    const url = new URL(request.url);
    const mes = url.searchParams.get("mes");

    const consultaBase = `
      SELECT p.mes, p.palabra_id, p.autor_id, p.puntaje, p.fecha_seleccion,
             w.palabra, w.significado
      FROM palabra_del_mes p
      JOIN words w ON w.id = p.palabra_id
    `;

    const fila = mes
      ? await env.DB.prepare(`${consultaBase} WHERE p.mes = ?`).bind(mes).first()
      : await env.DB.prepare(`${consultaBase} ORDER BY p.mes DESC LIMIT 1`).first();

    return json({ palabra_del_mes: fila || null });
  } catch (err) {
    return json({ error: "Error al obtener la palabra del mes", detalle: err.message }, 500);
  }
}

// Historial completo de Palabras del Mes, de la más reciente a la más
// antigua — para la pantalla/modal de historial en el frontend.
async function handleHistorialPalabraDelMes(request, env) {
  try {
    const url = new URL(request.url);
    const limite = Math.min(parseInt(url.searchParams.get("limite") || "24", 10) || 24, 100);
    ...
    return json({ error: "Error al obtener el historial", detalle: err.message }, 500);
  }
}

// Disparo manual (admin): recalcula el ranking del mes indicado (o el
// actual, si no se manda "mes") y corona su Palabra del Mes — sirve para
// probar sin esperar al Cron del día 1.
async function handleAdminElegirPalabraDelMes(request, env) {
  if (!checkAuth(request, env)) {
    return json({ error: "No autorizado" }, 401);
  }
  try {
    const body = await request.json().catch(() => ({}));
    const mes = body.mes || mesActualUTC();
    await calcularYGuardarRanking(env, mes);
    const resultado = await seleccionarPalabraDelMes(env, mes);
    return json({ ok: true, mes, ...resultado });
  } catch (err) {
    return json({ error: "Error al elegir la palabra del mes", detalle: err.message }, 500);
  }
}
