...
async function banearUsuarioConIPs(env, usuario_id, motivo, admin) {
  ...
  try {
    const explicacion = await generarExplicacionBaneo(env, motivo);
    if (explicacion) {
      await env.DB.prepare(
...
async function generarExplicacionBaneo(env, motivoAdmin) {
  const promptSistema =
    ...
    `a ese usuario, en español rioplatense (Argentina), la explicación de su sanción, para que la lea en ` +
    ...
    `Respondé ÚNICAMENTE con el texto de la explicación, sin comillas, sin JSON, sin encabezados ni firma.`;
  ...
  await registrarUsoClaude(env, { endpoint_origen: "explicacion_baneo", modelo: "claude-sonnet-4-6", usage: claudeData.usage });
...
async function handleEstadoBaneo(request, env) {
  try {
    ...
    const fila = await env.DB.prepare(
      `SELECT explicacion, fecha, apelacion_estado, apelacion_intentos FROM banned_users WHERE usuario_id = ?`
    ).bind(usuario_id).first();
